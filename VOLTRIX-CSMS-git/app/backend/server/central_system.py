...
from ocpp.v201.enums import (
    RegistrationStatusEnumType,       # Şarj istasyonunun sisteme kaydının kabul/ret durumunu belirtir
    AuthorizationStatusEnumType,      # Kullanıcının kimlik doğrulama durumunu belirtir (accepted/rejected)
    ReserveNowStatusEnumType,         # Rezervasyon talebinin durumunu belirtir (accepted/rejected)
    ConnectorStatusEnumType           # EVSE'nin durumunu belirtir (Available, Occupied, Reserved vs.)
)
...
class CentralSystem(CP):
    # Şarj İstasyonundan gelen BootNotification mesajını işler
    @on('BootNotification')
    ...
    # Şarj İstasyonundan gelen Authorize (kimlik doğrulama) talebini işler
    @on('Authorize')
    ...
    # Şarj İstasyonundan gelen durum bildirimini işler
    ...
    # Dış sistemden gelen rezervasyon isteğini işler
    @on('ReserveNow')
    ...
    # Şarj İstasyonundan gelen kalp atışı (heartbeat) mesajını işler
    @on('Heartbeat')
    ...
    # Şarj İstasyonundan gelen sayaç verilerini işler
    @on('MeterValues')
    ...
    # Şarj işleminin başlatılması ve bitirilmesi gibi TransactionEvent olaylarını işler
    @on('TransactionEvent')
    ...
    # Araç takıldığında (plug-in) simüle edilmiş bir durum bildirimi gönderir
    ...
    # Geçerli UTC zamanını döner (ISO formatında)
    def now(self):
        return datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z")

# Backend'den gelen rezervasyon isteğini uygun şarj istasyonuna gönderir
...
# WebSocket bağlantısı kurulduğunda çalışır, gelen Charge Point'i tanımlar
...
# CSMS WebSocket sunucusunu başlatır
